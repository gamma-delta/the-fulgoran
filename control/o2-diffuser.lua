...
--[[
  Each entity that cares about oxygen runs on ticks
]]
...
veh.on_nth_tick[10] = function(tick)
  ...
  local i = 0
  for _,assoc in pairs(tf_util.storage_table("oxygen-diffusers")) do
    if i % chunk_amt == chunk_tick_idx then
      ...
      if ff.error then
        ...
        if ff.catastrophe then
          ...
          o2lib.set_max_o2(diffuser, 99999999)
        else
          o2lib.set_max_o2(diffuser, 0)
        end
        tf_util.alert(diffuser, {type="fluid", name="pk-oxygen"}, ff.reason, true)
      else
        local max_o2 = #ff.ok * settings.global["pk-oxygen-volume-per-tile"].value
        o2lib.set_max_o2(diffuser, max_o2)
        -- give a little wiggle room
        local must_void = false
        if assoc.tank.fluidbox[1] then
          -- Add a little bit of headroom, because the pump is prone
          -- to overflow the tank
          must_void = max_o2 + 100 < assoc.tank.fluidbox[1].amount
          if must_void then
...
veh.events[defines.events.on_selected_entity_changed] = function(evt)
  ...
  local ff = tf_util.floodfill_o2(e.position, e.surface, e)
  if ff.error then
